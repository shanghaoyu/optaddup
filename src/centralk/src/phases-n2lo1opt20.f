...
cpot 是potential的简称，下面用external声明pot是个函数，所以如果不用bonn势call n3lo就行
      ...
      external pot
      real*8 phaseshifts(41,12),elab(41)
...
c     4                     j
  ...
  925 delta(iv)=delta(iv)*rd
      if (j1.eq.1)go to 927
      if (j1.eq.2)go to 928
      if (j1.eq.3)go to 929
  927 phaseshifts(k,1)=delta(1)
      phaseshifts(k,2)=delta(3)
      go to 926
  928 phaseshifts(k,3)=delta(1)
      phaseshifts(k,4)=delta(2)
      phaseshifts(k,5)=delta(3)
      phaseshifts(k,6)=delta(4)
      phaseshifts(k,7)=delta(5)
      go to 926
  929 phaseshifts(k,8)=delta(3)
      phaseshifts(k,9)=delta(4)
      phaseshifts(k,10)=delta(5) 
      phaseshifts(k,11)=delta(1)
      phaseshifts(k,12)=delta(2)
      go to 926
